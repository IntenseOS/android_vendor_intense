...
ifeq ($(filter RELEASE OFFICIAL WEEKLY SNAPSHOT EXPERIMENTAL,$(INTENSE_BUILDTYPE)),)
    INTENSE_BUILDTYPE :=
endif
...
ifeq ($(INTENSE_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        INTENSE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(INTENSE_BUILD)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            INTENSE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(INTENSE_BUILD)
        else
            INTENSE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(INTENSE_BUILD)
        endif
    endif
    ...
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        INTENSE_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(INTENSE_BUILDTYPE)$(INTENSE_EXTRAVERSION)-$(INTENSE_BUILD)
    else
